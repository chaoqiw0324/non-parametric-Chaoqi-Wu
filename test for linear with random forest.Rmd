---
title: "test for linear with random forest"
...
(p1_1_1+p1_1_2)/(p1_1_3+p1_1_4)/(p1_1_5+p1_1_6)
```
...
for (i in 1:nsample) {
  ...
  res_cb_L <- psi.hat_linear(Y = dat$Y,A = dat$A,L = L_vec, subset = NULL, out.bin = FALSE, exp.bin = TRUE, exp.scalar = TRUE)
  ...
  res_cb_rf_L <- psi.hat_rf(Y = dat$Y,A = dat$A,L = L_vec, subset = NULL, out.bin = FALSE, exp.bin = TRUE, exp.scalar = TRUE)
  ...
  res_cb_grf_L <- psi.hat_grf(Y = dat$Y,A = dat$A,L = L_vec, subset = NULL, out.bin = FALSE, exp.bin = TRUE, exp.scalar = TRUE)
...
table_cb_grf2 <- tibble(
  ...
  L <- c(sum(df_cb_grf_L2$contain)/nrow(df_cb_grf_L2))
  )
...
(p1_2_1+p1_2_2)/(p1_2_3+p1_2_4)/(p1_2_5+p1_2_6)
```
...
# result_cc2       <- result_cc      
# result_cc_L2     <- result_cc_L    
# result_cc_rf2    <- result_cc_rf   
# result_cc_rf_L2  <- result_cc_rf_L 
# result_cc_grf2   <- result_cc_grf  
# result_cc_grf_L2 <- result_cc_grf_L
for (i in 1:nsample) {
...
(p2_1_1+p2_1_2)/(p2_1_3+p2_1_4)/(p2_1_5+p2_1_6)
```
...
for (i in 1:nsample) {
  ...
  # L.true <- 2*L1 + 3*L2 + 4*L3
  Z <- 0.5 + 0.5*L.true
  Y.true <- 4*Z + rnorm(n,0,1)
  A.true <- -4*Z + rnorm(n,0,1)
  dat <- tibble(
    Y = Y.true,
    A = A.true
  ) %>% cbind(L_vec)
...
(p2_2_1+p2_2_2)/(p2_2_3+p2_2_4)/(p2_2_5+p2_2_6)
```
...
(p4_1_1+p4_1_2)/(p4_1_3+p4_1_4)/(p4_1_5+p4_1_6)
```
...
(p4_2_1+p4_2_2)/(p4_2_3+p4_2_4)/(p4_2_5+p4_2_6)
```
