...
for (i in 1:nsample) {
  ...
  # random forest
  res_cb_rf <- as.numeric(broom::tidy(glm(Y~A,data=dat,family = gaussian))[2,c(2,3)])
  ...
  res_cb_rf_L <- as.numeric(broom::tidy(glm(Y~A+L,data=dat,family = gaussian))[2,c(2,3)])
...
table_cb <- tibble(
  ...
  noL <- c(sum(df_cb$contain)/nrow(df_cb),round(mean(df_cb$point),2)),
  L <- c(sum(df_cb_L$contain)/nrow(df_cb_L),round(mean(df_cb_L$point),2))
  )
...
knitr::kable(table_cb,
             ...
             caption = "Result For Continuous Outcome to Binary Exposure(M-estimation)"
             ) 
...
table_cb_rf <- tibble(
  term <- c("Coverage Rate","Mean"),
  noL <- c(sum(df_cb_rf$contain)/nrow(df_cb_rf),round(mean(df_cb_rf$point),2)),
  L <- c(sum(df_cb_rf_L$contain)/nrow(df_cb_rf_L),round(mean(df_cb_rf_L$point),2))
  )
...
knitr::kable(table_cb_rf,
             ...
             digits = 3,
             col.names = c("Metric","No L","With L"),
             caption = "Result For Continuous Outcome to Binary Exposure(MLE)"
             ) 
...
for (i in 1:nsample) {
  ...
  #random forest
  res_cc_rf <- as.numeric(broom::tidy(glm(Y~A,data=dat,family = gaussian))[2,c(2,3)])
  ...
  res_cc_rf_L <- as.numeric(broom::tidy(glm(Y~A+L,data=dat,family = gaussian))[2,c(2,3)])
...
table_cc <- tibble(
  term <- c("Coverage Rate","Mean"),
  noL <- c(sum(df_cc$contain)/nrow(df_cc),round(mean(df_cc$point),2)),
  L <- c(sum(df_cc_L$contain)/nrow(df_cc_L),round(mean(df_cc_L$point),2))
  )
...
knitr::kable(table_cc,
             ...
             digits = 3,
             col.names = c("Metric","No L","With L"),
             caption = "Result For Continuous Outcome to Continuous Exposure(M-estimation)"
             )
...
table_cc_rf <- tibble(
  term <- c("Coverage Rate","Mean"),
  noL <- c(sum(df_cc_rf$contain)/nrow(df_cc_rf),round(mean(df_cc_rf$point),2)),
  L <- c(sum(df_cc_rf_L$contain)/nrow(df_cc_rf_L),round(mean(df_cc_rf_L$point),2))
  )
...
knitr::kable(table_cc_rf,
             ...
             digits = 3,
             col.names = c("Metric","No L","With L"),
             caption = "Result For Continuous Outcome to Continuous Exposure(MLE)"
             )
...
for (i in 1:nsample) {
  ...
  res_bc_L <- psi.hat_linear(dat$Y,dat$A,L, subset = NULL, out.bin = TRUE, exp.bin = FALSE, exp.scalar = TRUE)
  ...
  # random forest
  res_bc_rf <- as.numeric(broom::tidy(glm(Y~A,data=dat,family = binomial))[2,c(2,3)])
  ...
  res_bc_rf_L <- as.numeric(broom::tidy(glm(Y~A+L,data=dat,family = binomial))[2,c(2,3)])
...
for (i in 1:nsample) {
  ...
  #random forest
  res_bb_rf <- as.numeric(broom::tidy(glm(Y~A,data=dat,family=binomial))[2,c(2,3)])
  ...
  res_bb_rf_L <- as.numeric(broom::tidy(glm(Y~A+L,data=dat,family=binomial))[2,c(2,3)])
...
table_bb <- tibble(
  term <- c("Coverage Rate","Mean"),
  noL <- c(sum(df_bb$contain)/nrow(df_bb),round(mean(df_bb$point),2)),
  L <- c(sum(df_bb_L$contain)/nrow(df_bb_L),round(mean(df_bb_L$point),2))
  )
...
knitr::kable(table_bb,
             ...
             digits = 3,
             col.names = c("Metric","No L","With L"),
             caption = "Result For Binary Outcome to Binary Exposure(M-estimation)"
             )
...
table_bb_rf <- tibble(
  term <- c("Coverage Rate","Mean"),
  noL <- c(sum(df_bb_rf$contain)/nrow(df_bb_rf),round(mean(df_bb_rf$point),2)),
  L <- c(sum(df_bb_rf_L$contain)/nrow(df_bb_rf_L),round(mean(df_bb_rf_L$point),2))
  )
...
knitr::kable(table_bb_rf,
             ...
             digits = 3,
             col.names = c("Metric","No L","With L"),
             caption = "Result For Binary Outcome to Binary Exposure(MLE)"
             )
